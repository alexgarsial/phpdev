...
<head>
    ...
    <link
        ...
        href="/public/css/styles.mobile.css" />
            ...
            <div class="newsfeed-container">
                <ul class="category-container">
                    ...
                    <?php foreach ($categories as $catName => $catNum): ?>
                        <li class=<?= $selectedCat === $catName ? 'cat-active' : '' ?>><a href="/?cat=<?= $catName ?>"><?= ucfirst(strtolower($catName)) ?><span class="small">(<?= ($catNum) ?>)</span></a></li>
                    <?php endforeach ?>
                </ul>
                <div class="newsfeed-content">
                    <?php if (!$selectedCat) : ?>
                        <?php foreach ($categories as $cat => $num): ?>
                            <h2><?= ucfirst(strtolower($cat)) ?></h2>
                        ...
                        <?php endforeach ?>
                    <?php else : ?>
                        <h2><?= ucfirst(strtolower($selectedCat)) ?></h2>
                        <div class="articles-container">
